...
export default function GameTaskPage() {
  ...
  const diceControlsTop = useAnimation();
  const diceControlsBottom = useAnimation();
  ...
  useEffect(() => {
    ...
    const preloadImages = [...COMBO_GIFS, ...allFlavors.map((f) => f.img), task1, task2, task3, Transition, TopDice, BottomDice].map(
      (src) =>
        new Promise((resolve) => {
          const img = new Image();
          img.onload = () => {
  ...
  // Animate dice with smooth bouncing motion, only when dice are visible
  useEffect(() => {
    if (currentTask === 1 && !gameOver && !gameActive && !activeSection) {
      ...
      // Reset dice animations when not visible
      diceControlsTop.start({ opacity: 0, y: 20, scale: 0.8 });
      diceControlsBottom.start({ opacity: 0, y: 20, scale: 0.8 });
    }
  }, [diceControlsTop, diceControlsBottom, currentTask, gameOver, gameActive, activeSection]);
        ...
        {currentTask > 3 && !activeSection && (
          <div className="flex flex-col items-center pt-20">
            <div className="flex flex-col items-center mb-4">
              <span className="w-28 px-2 py-1 border border-orange-500 text-center text-2xl font-extrabold rounded-lg text-white">
                Final Score
              </span>
